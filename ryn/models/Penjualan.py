...
class Penjualan(models.Model):
    ...
    detailpenjualan_ids = fields.One2many(
        comodel_name='ryn.detailpenjualan',
    ...
    def _compute_totalbayar(self):
        for rec in self:
            a = sum(self.env['ryn.detailpenjualan'].search([('penjualan_id','=',rec.id)]).mapped('subtotal'))
            rec.total_bayar = a
    ...
    def _ondelete_penjualan(self):
        if self.filtered(lambda line: line.state != 'draft'): 
            ...
            if self.detailpenjualan_ids:
                a=[]
                for rec in self:
                    a = self.env['ryn.detailpenjualan'].search([('penjualan_id','=',rec.id)])
                    print(a)
                for ob in a:
    ...
    def write(self,vals):
        ...
        for rec in self:
            b = self.env['ryn.detailpenjualan'].search([('penjualan_id','=',rec.id)])
            print(a)
            print(b)
            for databaru in b:
                if databaru in a:
...
class DetailPenjualan(models.Model):
    _name = 'ryn.detailpenjualan'
    _description = 'New Description'
    
    name = fields.Char(string='Nama')
    penjualan_id = fields.Many2one(comodel_name='ryn.penjualan', string='Detail Penjualan')
    barang_id = fields.Many2one(comodel_name='ryn.barang', string='List Barang')
    ...
    def create(self,vals):
        ...
        if record.qty:
            self.env['ryn.barang'].search([('id','=',record.barang_id.id)]).write({'stok' : record.barang_id.stok-record.qty})
            return record
    ...
    # _sql_constraints = [
    #     ('qty_wrong','check(qty<1)','Quantity tidak boleh kurang dari 1')
    # ]